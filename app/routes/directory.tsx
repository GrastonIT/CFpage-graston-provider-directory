...
import { providers as allProviders, Provider, GrastonLevel } from '../data/providersEnhanced';
...
export default function DirectoryRoute() {
  ...
  const ViewToggleButton = ({ mode, label }: { mode: ViewMode; label: string }) => (
    <button
      ...
      className={`p-2 rounded-md transition-colors ${
        viewMode === mode ? 'bg-[var(--graston-teal)] text-white' : 'hover:bg-gray-200'
      }`}
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-gray-50 overflow-hidden">
      {/* Filters Sidebar */}
      <aside
        className={`fixed lg:relative inset-y-0 left-0 z-20 w-80 bg-white shadow-lg transform ${
